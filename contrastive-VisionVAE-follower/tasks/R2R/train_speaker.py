...
from env import R2RBatch, ImageFeatures  # 加载R2R数据集
from model import SpeakerEncoderLSTM, SpeakerDecoderLSTM
from speaker import Seq2SeqSpeaker
import eval_speaker

from vocab import SUBTRAIN_VOCAB, TRAIN_VOCAB, TRAINVAL_VOCAB  # 加载语料库
...
def train(args, train_env, agent, log_every=log_every, val_envs=None):
    ...
    data_log = defaultdict(list)  # 存训练过程中得到的loss等数据
    start = time.time()

    split_string = "-".join(train_env.splits)

    def make_path(n_iter):  # TODO 写错了？n_iters？
        return os.path.join(
    ...
    for idx in range(0, args.n_iters, log_every):
        ...
        for env_name, (val_env, evaluator) in sorted(val_envs.items()):
            ...
            # Get validation distance from goal under evaluation conditions在评估条件下获得与目标的验证距离
