...
def entry_point(args):
    ...
    pragmatic_speaker = (args.follower_model_prefix is not None)  # 在训练follower的时候也会用到speaker
    if pragmatic_speaker:
        ...
        follower = None  # 只进行数据增强，不训练follower
        ...
        if pragmatic_speaker:
            candidate_lists_by_instr_id = \
                rational_speaker.generate_and_score_candidates(
                    ...
                    args.rational_speaker_n_candidates, include_gold=False)  # 得到预测指令
...
def make_arg_parser():
    ...
    parser.add_argument(
        "--rational_speaker_n_candidates", type=int, default=40)  # beam-search中的参数k=40
    return parser
