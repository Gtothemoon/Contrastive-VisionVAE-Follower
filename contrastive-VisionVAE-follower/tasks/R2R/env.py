...
from utils import load_datasets, load_nav_graphs, structured_map, vocab_pad_idx, decode_base64, k_best_indices, \
    try_cuda, spatial_feature_from_bbox
...
def _build_action_embedding(adj_loc_list, features):
    ...
    for a, adj_dict in enumerate(adj_loc_list):  # a表示前往哪一个视点，a=0表示stop
        if a == 0:
            # the embedding for the first action ('stop') is left as zero
            continue
        embedding[a, :feature_dim] = features[adj_dict['absViewIndex']]  # 找到前往视点的全景图像特征
...
_static_loc_embeddings = [
    build_viewpoint_loc_embedding(viewIndex) for viewIndex in range(36)]  # (36, 36, 128)
...
def _adjust_heading(sim, heading):
    ...
    for _ in range(int(abs(heading))):
        # sim.makeAction(0, np.sign(heading), 0)  # np.sign()阶跃函数，改用下一行
        sim.makeAction([0], [np.sign(heading)], [0])  # TODO 模拟器版本改动


def _adjust_elevation(sim, elevation):
    for _ in range(int(abs(elevation))):
        ''' Make possibly more than one elevation turns '''
        # sim.makeAction(0, 0, np.sign(elevation))  # 改用下一行，大部分改动应该是套一个list类型
        sim.makeAction([0], [0], [np.sign(elevation)])  # TODO 模拟器版本改动


# TODO 高级动作
def _navigate_to_location(sim, nextViewpointId, absViewIndex):
    state = sim.getState()
    state = state[0]  # TODO 模拟器版本改动
    ...
    for n_loc, loc in enumerate(state.navigableLocations):
        if loc.viewpointId == nextViewpointId:  # TODO 在模拟器中找到下一个前往视点
            a = n_loc
            next_loc = loc
            break
    ...
    # 3. Take action
    # sim.makeAction(a, 0, 0)  # 在模拟器中前往下一个视点
    sim.makeAction([a], [0], [0])  # TODO 模拟器版本改动


# TODO 得到当前状态和全景可达视点信息
def _get_panorama_states(sim):
    ...
    feature[12]始终是水平向前的图像的特征
    """
    state = sim.getState()  # 获得模拟器当前state
    state = state[0]  # TODO 模拟器版本改动
    initViewIndex = state.viewIndex  # absViewIndex
    # 1. first look down, turning to relViewIndex 0
    elevation_delta = -(state.viewIndex // 12)
    _adjust_elevation(sim, elevation_delta)  # 先指向地面
    ...
    for relViewIndex in range(36):
        ...
        for loc in state.navigableLocations[1:]:  # 第一个 navigableLocations 是当前视点自身
            ...
            if (loc.viewpointId not in adj_dict or
                ...
                adj_dict[loc.viewpointId] = {
                    'absViewIndex': absViewIndex,  # 当前视点指向下一视点的视角
                    'nextViewpointId': loc.viewpointId,  # 下一视点
        ...
        if (relViewIndex + 1) % 12 == 0:
            ...
            sim.makeAction([0], [1], [1])  # TODO 模拟器版本改动
        else:
            # sim.makeAction(0, 1, 0)  # Turn right
            sim.makeAction([0], [1], [0])  # TODO 模拟器版本改动
    # 3. turn back to the original view
    _adjust_elevation(sim, - 2 - elevation_delta)
    # TODO 回到初始状态
    state = sim.getState()
    state = state[0]  # TODO 模拟器版本改动
    assert state.viewIndex == initViewIndex  # TODO check the agent is back
    # collect navigable location list
    stop = {
        'absViewIndex': -1,
        'nextViewpointId': state.location.viewpointId}  # 当前state视点自身
...
BottomUpViewpoint = namedtuple("BottomUpViewpoint",
                               ["cls_prob", "image_features", "attribute_indices", "object_indices", "spatial_features",
                                "no_object_mask"])


def load_world_state(sim, world_state):  # world_state是字典
    scanId = [world_state[0]]  # TODO tuple类型只能通过数字索引！
    viewpointId = [world_state[1]]
    heading = [world_state[2]]
    elevation = [world_state[3]]
    # sim.newEpisode(*world_state)  # *world_state变成4个参数，TODO 报错，每个参数都要是list类型，改用下一句
    sim.newEpisode(scanId, viewpointId, heading, elevation)  # TODO 模拟器版本改动
...
def make_sim(image_w, image_h, vfov):
    ...
    # sim.init()  # TODO 报错没有这个函数！用下一句
    sim.initialize()  # TODO 模拟器版本改动
    return sim
...
class ImageFeatures(object):
    ...
    def from_args(args):
        feats = []
        for image_feature_type in sorted(args.image_feature_type):  # 默认是"mean_pooled"
...
class MeanPooledImageFeatures(ImageFeatures):
    def __init__(self, image_feature_datasets):
        ...
        self.mean_pooled_feature_stores = [paths.mean_pooled_feature_store_paths[dataset]
                                           for dataset in image_feature_datasets]  # 文件路径
            ...
            with open(mpfs, "rt") as tsv_in_file:
                ...
                for item in reader:
                    ...
                    features = np.frombuffer(decode_base64(item['features']), dtype=np.float32).reshape(
                        (ImageFeatures.NUM_VIEWS, ImageFeatures.MEAN_POOLED_DIM))  # (36 x 2048)
        ...
        self.features = {
            long_id: np.concatenate(feats, axis=1)  # TODO 不懂
    ...
    def get_features(self, state):
        ...
        # TODO Return feature of all the 36 views
...
class ConvolutionalImageFeatures(ImageFeatures):
    ...
    def __init__(self, image_feature_datasets, split_convolutional_features=True,
                 downscale_convolutional_features=True):
        self.image_feature_datasets = image_feature_datasets
    ...
    def _get_convolutional_features(self, scanId, viewpointId, viewIndex):
        ...
        for cfs in self.convolutional_feature_stores:
            if self.split_convolutional_features:
                path = os.path.join(cfs, scanId, "{}_{}{}.npy".format(viewpointId, viewIndex,
                                                                      "_downscaled" if self.downscale_convolutional_features else ""))
...
class BottomUpImageFeatures(ImageFeatures):
    ...
    def __init__(self, number_of_detections, precomputed_cache_path=None, precomputed_cache_dir=None, image_width=640,
                 image_height=480):
    ...
    def _get_viewpoint_features(self, scan_id, viewpoint_id):
        ...
        for viewpoint in data:
            ...
            spatial_feats[0:len(top_indices)] = spatial_feature_from_bbox(viewpoint['boxes'][top_indices],
            ...
            viewpoint_features.append(
                BottomUpViewpoint(cls_prob, image_features, attribute_indices, object_indices, spatial_feats,
                                  no_object))
...
class EnvBatch():
    ...
    def getStates(self, world_states, beamed=False):
        ...
        def f(sim, world_state):
            load_world_state(sim, world_state)  # TODO 在模拟器中加载状态
            return _get_panorama_states(sim)  # TODO 返回全景信息
    ...
    def makeActions(self, world_states, actions, last_obs, beamed=False):
        ...
        def f(sim, world_state, action, last_ob):
            ...
            return get_world_state(sim)  # 返回新的state
...
class R2RBatch():
    ...
    def _load_nav_graphs(self):
        ...
        self.paths = {}  # 加权无向图中两点间的最短路径
        for scan, G in self.graphs.items():  # compute all shortest paths
            self.paths[scan] = dict(nx.all_pairs_dijkstra_path(G))
        self.distances = {}  # 加权无向图中两点间的最短距离
        for scan, G in self.graphs.items():  # compute all shortest paths
    ...
    def _next_minibatch(self, sort_instr_length):
        ...
        if sort_instr_length:
            batch = sorted(batch, key=lambda item: item['instr_length'], reverse=True)  # 按句子长度从大到小排序
        self.batch = batch
    ...
    # TODO 得到真实action（相当于label）
    def _shortest_path_action(self, state, adj_loc_list, goalViewpointId):  # adj_loc_list：当前视点的邻接点
        """
        ...
        for n_a, loc_attr in enumerate(adj_loc_list):
            if loc_attr['nextViewpointId'] == nextViewpointId:
                return n_a  # TODO 高级action

        # TODO Next nextViewpointId not found! This should not happen!
    ...
    def observe(self, world_states, beamed=False, include_teacher=True):
        ...
        for i, states_beam in enumerate(self.env.getStates(world_states, beamed=beamed)):
            ...
            for state, adj_loc_list in states_beam if beamed else [states_beam]:
                assert item['scan'] == state.scanId  # TODO batch的信息和模拟器中的信息是对应的
                # TODO 当前state的36个视角图像特征，列表[[1,2,3,4]]的len()==1
                ...
                # 横向连接，(36 x 2048) + (36 x 4*32) = (36 x 2048+128)
                feature_with_loc = np.concatenate((feature[0], _static_loc_embeddings[state.viewIndex]),
                                                  axis=-1)  # v_t，当前视点36个全景图像特征
                action_embedding = _build_action_embedding(adj_loc_list, feature[0])  # u_t，当前视点可以采取的action
                ob = {
                    ...
                    'heading': state.heading,
                    'elevation': state.elevation,
                    'feature': [feature_with_loc],  # 当前视点的v_t
                    'step': state.step,
                    'adj_loc_list': adj_loc_list,  # 当前视点可以选择的动作（高级action）
                    'action_embedding': action_embedding,  # 当前视点的u_t
                    'navigableLocations': state.navigableLocations,  # 当前视点可以到达的视点
                    'instructions': item['instructions'],
                }
                if include_teacher:
                    ob['teacher'] = self._shortest_path_action(state, adj_loc_list, item['path'][-1])  # 真实动作label
    ...
    # TODO 获取下一个batch
    def reset(self, sort=False, beamed=False, load_next_minibatch=True):  # sort=True
        """ Load a new minibatch / episodes. """
        if load_next_minibatch:
            self._next_minibatch(sort)
        assert len(self.batch) == self.batch_size
        return self.get_starting_world_states(self.batch, beamed=beamed)  # 返回当前batch的初始states

    def step(self, world_states, actions, last_obs, beamed=False):
        """ Take action (same interface as makeActions) """
        # TODO 在模拟器中执行action，返回新的states
    ...
    def gold_obs_actions_and_instructions(self, max_steps, load_next_minibatch=True):
        starting_world_states = self.reset(load_next_minibatch=load_next_minibatch)  # TODO 获取下一个batch
