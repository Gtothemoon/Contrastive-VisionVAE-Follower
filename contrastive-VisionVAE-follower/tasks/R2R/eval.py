...
class Evaluation(object):
    ...
    # TODO 预测轨迹不一定能准确的到达目的地，返回预测轨迹中距离目的地最近的视点（不一定是最后一个视点）
    def _get_nearest(self, scan, goal_id, path):  # TODO path是预测轨迹
        near_id = path[0][0]  # 预测轨迹的起点
        near_d = self.distances[scan][near_id][goal_id]  # 真实轨迹中起点到终点的最短路径长度
        for item in path:
            d = self.distances[scan][item[0]][goal_id]  # 真实轨迹中视点i到终点的最短路径长度
            if d < near_d:
                near_id = item[0]
                near_d = d
        return near_id

    def _score_item(self, instr_id, path):  # TODO path是预测轨迹
        """ Calculate error based on the final position in trajectory, and also
            the closest position (oracle stopping rule). """
        gt = self.gt[int(instr_id.split('_')[0])]
        start = gt['path'][0]  # 真实轨迹的起点
        assert start == path[0][0], 'Result trajectories should include the start position'
        goal = gt['path'][-1]  # 真实轨迹的真实终点
        final_position = path[-1][0]  # 预测轨迹的预测终点
        nearest_position = self._get_nearest(gt['scan'], goal, path)  # 预测轨迹中距离真实目的地最近的视点
        # TODO 预测轨迹的终点和真实轨迹的终点的距离
        nav_error = self.distances[gt['scan']][final_position][goal]
        # TODO 预测轨迹中距离真实轨迹的终点的最近距离
        oracle_error = self.distances[gt['scan']][nearest_position][goal]
        trajectory_steps = len(path) - 1
        trajectory_length = 0  # 计算预测轨迹的路径长度
        prev = path[0]  # 预测轨迹的起点
        for curr in path[1:]:
    ...
    def score_results(self, results):
        ...
        for instr_id, result in results.items():
            if instr_id in instr_ids:
                ...
                if 'score' in result:
                    # model_scores.append(result['score'])  # 报错，result['score']里是一个tensor标量，必须先取出标量数值，才能平均
        ...
        assert float(oracle_successes) / float(len(self.scores['oracle_errors'])) == score_summary[
            'oracle_rate']  # NoQA
